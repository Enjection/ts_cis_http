        ...
        rapidjson::Document::AllocatorType& allocator)
{
    ...
    for(auto& [project, jobs] : projects->projects)
    {
        if(auto perm = rights->check_project_right(ctx.username, project.name);
                perm.has_value() && perm.value().read)
        {
            array.PushBack(
        ...
        rapidjson::Document::AllocatorType& allocator)
{
    ...
    if(permitted)
    {
        ...
        rapidjson::Value array_value;
        array.SetArray();
        for(auto [perm_name, val] : permissions)
        {
            array_value.SetObject(); //FIXME
            array.PushBack(array_value, allocator);
        }
        response_data.AddMember("users", array, allocator);
        return std::nullopt;
        ...
        rapidjson::Document::AllocatorType& allocator)
{
    ...
    if(permitted)
    {
        for(auto& value : request_data["permissions"].GetArray())
        {
            if(!value.IsObject())
            {
                return "Invalid JSON.";
            }
            auto project_name = get_string(value, "project");
            auto read = get_bool(value, "read");
            auto write = get_bool(value, "write");
            auto execute = get_bool(value, "execute");
